...
module.exports = {
  ...
  */
  run: async (client, message, args) => {
    ...
    if (balance < 1000) return message.reply({ content: `You do not have enough Spirit Coins to summon. You require \`1000\` Spirit Coins to summon.` })
    ...
    if (convertitems == false) {
      ...
      const embed = new MessageEmbed()
        ...
        .setImage(`https://cdn.discordapp.com/attachments/761545761451802685/1006135736090763374/ezgif.com-gif-maker_14.gif?size=4096`)
  ...
  const embeda = new MessageEmbed()
    .setColor('RANDOM')
    .setDescription(`\nYou have successfully summoned \`${spiritStar}\` **${summoned} Shards**!\n`)
    .setTitle(`Successful Summoning!`)
    .setFooter({ text: message.author.tag, iconURL: message.author.displayAvatarURL({ dynamic: true }) })
    .setImage(spiritImages[summoned])
  const sus = await message.reply({ embeds: [embed] })
