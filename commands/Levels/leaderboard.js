...
const { embedPages } = require('simply-djs');
module.exports = {
  name: 'leaderboard',
  aliases: ["lb"],
  ...
  run: async (client, message, args) => {
    await xp.leaderboard(client, message.guild.id, 40).then(board => {
      ...
      board.forEach((user, index) => {
        if (index + 1 <= 10) {
          a.push(`**${index + 1}.** ${user.username} » **Level:** \`${user.level}\` | **XP:** \`${user.shortxp}\``)
        } else if (index + 1 > 10 && index + 1 <= 20) {
          b.push(`**${index + 1}.** ${user.username} » **Level:** \`${user.level}\` | **XP:** \`${user.shortxp}\``)
        } else if (index + 1 > 20 && index + 1 <= 30) {
          c.push(`**${index + 1}.** ${user.username} » **Level:** \`${user.level}\` | **XP:** \`${user.shortxp}\``)
        }
      })
...
embedPages(client, message, pg, {
          timeout: 60000,
          firstEmoji: "911637785037910077",
          backEmoji: "911640280434892850",
          forwardEmoji: "911640141007839273",
          lastEmoji: "911637840176250890",
          btncolor: "SUCCESS",
          delcolor: "DANGER",
          skipcolor: "PRIMARY",
          skipBtn: true,
