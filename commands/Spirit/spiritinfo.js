...
module.exports = {
    name: "myspiritinfo",
    aliases: ['msi'],
    description: `Check your selected spirit's information.`,
    timeout: 10,
    /**
     *
     * @param {Client} client
    ...
    run: async (client, message, args) => {
        ...
        const spiritID = args[0] || profileSearch.selected
        const spiritSearch = await spiritSchema.findOne({ id: spiritID })
        if(!spiritSearch) return message.reply({content: `That is not a valid id.`})
        ...
        const hep = Math.floor(spiritStats.hp - spiritST.hp)
        const attac = Math.floor(spiritStats.strength - spiritST.strength)
        const defenc = Math.floor(spiritStats.strength - spiritST.strength)
        const agile = Math.floor(spiritStats.agility - spiritST.agility)
        const embed = new MessageEmbed()
        .setColor(profileSearch.color || '#ff0000')
        .addFields(
            {
                name: `Name`,
                value: `${spiritSearch.name} 【${'<a:starSpin:1006138461234937887>'.repeat(spiritSearch.stars)}】`
            },
            {
                name: 'HP',
                value: `${Math.floor(spiritStats.hp)} **[+${hep >= 0 ? hep : 0}]**`
            },
            {
                name: 'Strength',
                value: `${Math.floor(spiritStats.strength)} **[+${attac >= 0 ? attac : 0}]**`
            },
            {
                name: 'Defence',
                value: `${Math.floor(spiritStats.defence)} **[+${defenc >= 0 ? defenc : 0}]**`
            },
            {
                name: `Agility`,
                value: `${Math.floor(spiritStats.agility)} **[+${agile >= 0 ? agile : 0}]**`
            },
            {
                name: `Abilities`,
            ...
            {
                name: `Happiness`,
                value: `${progressBar(spiritSearch.happiness, 100, 10)} **[${Math.floor(spiritSearch.happiness)}/100]**`
            }
        )
        .setImage(spiritImages[spiritSearch.name])
        .setFooter({text: `ID: ${spiritID} | ${spiritST.quotes}`})
